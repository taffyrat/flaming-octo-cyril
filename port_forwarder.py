...
def createForwardingThreads(externalSocket, internalSocket):
	...
	externalToInternalThread.join()
	internalToExternalThread.join()

	# Close the sockets once the threads are finished.
...
def forwardPackets(readSocket, writeSocket):
	...
	while True:
		...
		# If we get an exception, check to see if it is because there is no data to read.
	        ...
	        if socketError.args[0] == errno.EWOULDBLOCK:
	        	continue
	        else:
	        	print("Error occured on recv: {0}".format(socketError))
